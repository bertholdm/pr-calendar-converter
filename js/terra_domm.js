function terra_domm(typ, jahr, mona, tag, std, min, sec) {
    ...
    /*
                                    ...
                                    double precision  zeit,time,dif,domm,dom,rhyn,rhy,kado,kad,croz
                                    double precision  cro,hiddyn,hid
    ...
    /*
                                  'Programm zur Umrechnung von Daten ATZ/NGZ nach Domm'
    ...
    // Rechnet um in Sekunden seit 1 Domm
    time = zeit + dif;
    if (time >= 0) {
        domm = Math.floor(time / dom);
        rhyn = Math.floor((time - domm * dom) / rhy);
        kado = Math.floor((time - domm * dom - rhyn * rhy) / kad);
        croz = Math.floor((time - domm * dom - rhyn * rhy - kado * kad) / cro);
        hiddyn = Math.floor(
            (time - domm * dom - rhyn * rhy - kado * kad - croz * cro) / hid
        );
        return (
            (croz + 1).toString() +
            "C " +
    ...
    // Berechnet Daten kleiner 1 Domm
    if (time < 0) {
        time = Math.abs(time);
        domm = -Math.floor(time / dom);
        time = time - domm * dom;
        rhyn = Math.floor((dom - time) / rhy);
        kado = Math.floor((dom - time - rhyn * rhy) / kad);
        croz = Math.floor((dom - time - rhyn * rhy - kado * kad) / cro);
        hiddyn = Math.floor(
            (dom - time - rhyn * rhy - kado * kad - croz * cro) / hid
        );
        return (
            (croz + 1).toString() +
            "C " +
