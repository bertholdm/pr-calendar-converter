...
* Nimmt die Eingabewerte an und ueberprueft, ob sie zulaessig sind.
      ...
      print *, 'Programm zur Umrechnung von Daten ATZ/NGZ nach Domm'
...
! Rechnet um in Sekunden seit 1 Domm
      ...
      if (time >= 0) then
         domm = int(time/dom)
         rhyn = int((time-domm*dom)/rhy)
         kado = int((time-domm*dom-rhyn*rhy)/kad)
         croz = int((time-domm*dom-rhyn*rhy-kado*kad)/cro)
         hiddyn = int((time-domm*dom-rhyn*rhy-kado*kad-croz*cro)/hid)
         print *, croz+1, 'C', kado+1, 'K', rhyn+1, 'R /', domm+1, 'D'
         print *, ''
         print *, int(hiddyn), ' H'
      endif
! Berechnet Daten kleiner 1 Domm
        if (time < 0) then
         ...
         domm = -int(time/dom)
         time = time-domm*dom
         rhyn = int((dom-time)/rhy)
         kado = int((dom-time-rhyn*rhy)/kad)
         croz = int((dom-time-rhyn*rhy-kado*kad)/cro)
         hiddyn = int((dom-time-rhyn*rhy-kado*kad-croz*cro)/hid)
         print *, croz+1, 'C', kado+1, 'K', rhyn+1, 'R /', domm-1, 'D'
         print *, ''
         print *, int(hiddyn), ' H'
        endif
